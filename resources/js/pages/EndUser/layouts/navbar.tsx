...
const Navbar: React.FC = () => {
    ...
    const handleLogout = () => {
        const logoutRoute = auth?.user?.is_admin ? '/logout' : '/student/logout';
    ...
    return (
        <>
                        ...
                        <ul className="flex items-center space-x-2">
                            ...
                            {auth?.user ? (
                                ...
                                >
                                    <button
                                        type="button"
                                        className="inline-flex items-center gap-2 rounded-full bg-color-primary/10 px-4 py-2 text-color-primary font-semibold transition-all duration-300 hover:bg-color-primary hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-color-primary"
                                    >
                                        ...
                                        <span className="truncate max-w-[120px] text-sm font-semibold">{auth.user.name}</span>
                                        <i className="far fa-chevron-down text-xs"></i>
                                    </button>

                                    {userMenuOpen && (
                                        <ul
                                            className="absolute left-0 top-full z-50 mt-2 w-44 rounded-2xl border border-gray-100 bg-white p-1 shadow-[0_15px_35px_rgba(15,23,42,0.25)] transition-all duration-200 ease-out"
                                            onMouseEnter={openUserMenu}
                                            onMouseLeave={closeUserMenu}
                                        >
                                            <li>
                        ...
                        <div className="hidden flex-1 justify-center lg:flex">
                            <ul className="flex items-center space-x-6">
                                {mainMenu.map((menuItem) => (
                                    <li key={menuItem.link} className="group relative">
                                        <a
                                            className={`relative text-lg font-semibold capitalize text-gray-900 transition-all duration-300 hover:-translate-y-0.5 hover:scale-105 hover:text-color-primary hover:drop-shadow-lg font-playpen-arabic ${
                                        ...
                                        {menuItem.child && (
                                            <ul className="invisible absolute left-0 top-full z-10 w-64 rounded-md border border-gray-100 bg-white opacity-0 shadow-xl transition-all duration-300 ease-in-out group-hover:visible group-hover:opacity-100">
            ...
            {mobileOpen && (
                <div
                    dir="ltr"
                    className="absolute left-0 z-[997] w-full bg-white shadow-lg lg:hidden"
            ...
            >
                <form action="/search" method="GET" className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
                    <button
                        type="button"
