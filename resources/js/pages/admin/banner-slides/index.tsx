...
export default function BannerSlidesIndex({ slides, stats }: { slides: PaginatedData<BannerSlide>; stats: Stats }) {
    const onDelete = async (id: number) => {
        if (!(await confirmDelete())) {
            return;
        }

        router.delete(`/admin/banner-slides/${id}`);
    };

    return (
        <AdminLayout title="شرائح الواجهة">
            <div className="space-y-6">
                <section className="rounded-[28px] border border-orange-100 bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_52%,#fff1e6_100%)] p-6 shadow-sm">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="text-right">
                            <div className="text-xs font-bold tracking-[0.3em] text-orange-500">الواجهة الرئيسية</div>
                            <h1 className="mt-2 text-2xl font-black text-slate-900">إدارة شرائح البانر</h1>
                            <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-600">
                                نظّم الشرائح الظاهرة في أعلى الصفحة الرئيسية، وتحكم في ترتيبها وصورتها وحالتها بشكل واضح.
                            </p>
                        </div>
                        <Link
                            href="/admin/banner-slides/create"
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    <div className="rounded-3xl border border-slate-200 bg-white p-5 text-right shadow-sm">
                        <div className="text-sm font-semibold text-slate-500">إجمالي الشرائح</div>
                    ...
                    <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-5 text-right shadow-sm">
                        <div className="text-sm font-semibold text-emerald-700">الشرائح النشطة</div>
                    ...
                    <div className="rounded-3xl border border-orange-100 bg-orange-50 p-5 text-right shadow-sm">
                        <div className="text-sm font-semibold text-orange-700">أعلى ترتيب حالي</div>
                ...
                <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-6 py-4 text-right">
                        <h2 className="text-lg font-bold text-slate-900">قائمة الشرائح</h2>
                        <p className="mt-1 text-sm text-slate-500">يمكنك تعديل المحتوى أو حذف أي شريحة من هنا.</p>
                    </div>
                        ...
                        <table className="min-w-full text-sm">
                            <thead className="bg-slate-50 text-right text-slate-600">
                                <tr>
                                    ...
                                    <th className="px-4 py-3">العنوان الفرعي</th>
                            ...
                            <tbody>
                                {slides.data.map((slide) => (
                                    <tr key={slide.id} className="border-t border-slate-100 align-top">
                                        <td className="px-4 py-4 font-medium text-slate-500">{slide.id}</td>
                                        <td className="px-4 py-4">
                                            <div className="text-right">
                                                ...
                                                <div className="mt-1 line-clamp-2 text-xs leading-6 text-slate-500">
                                                    {slide.description || 'لا يوجد وصف مضاف لهذه الشريحة.'}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-slate-600">{slide.sub_title ?? 'غير متوفر'}</td>
                                        <td className="px-4 py-4">
                                            {slide.background_image_url ? (
                                                <img
                                                    src={slide.background_image_url}
                                                    alt={slide.title}
                                                    className="h-14 w-24 rounded-2xl object-cover shadow-sm"
                                                />
                                            ) : (
                                                <span className="text-slate-400">بدون صورة</span>
                                            )}
                                        </td>
                                        <td className="px-4 py-4">
                                            <span
                                            ...
                                            >
                                                {slide.status ? 'نشطة' : 'غير نشطة'}
                                            </span>
                                        </td>
                                        <td className="px-4 py-4 font-semibold text-slate-700">{slide.sort_order}</td>
                                        <td className="px-4 py-4">
                                            <div className="flex items-center justify-start gap-3">
                                                <Link
                                                    href={`/admin/banner-slides/${slide.id}/edit`}
                ...
                <PaginationLinks links={slides.links} />
            </div>
        </AdminLayout>
    );
}
