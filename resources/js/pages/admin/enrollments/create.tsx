...
export default function EnrollmentCreate({
    ...
    return (
        <AdminLayout title="إضافة طلب تسجيل">
            <div className="mx-auto w-full max-w-4xl space-y-5">
                <div className="flex items-center justify-between">
                    <div className="text-right">
                        <h1 className="text-2xl font-black text-slate-900">إضافة طلب تسجيل</h1>
                        <p className="mt-1 text-sm text-slate-500">اختر الطالب والكورس وحدد تاريخ التسجيل إن لزم.</p>
                    </div>
                ...
                <form onSubmit={onSubmit} className="grid gap-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <button type="submit" disabled={processing} className="w-full rounded-2xl bg-orange-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-orange-500 disabled:opacity-50">حفظ طلب التسجيل</button>
                </form>
            </div>
        </AdminLayout>
    );
