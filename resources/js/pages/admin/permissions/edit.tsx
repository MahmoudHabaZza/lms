...
export default function PermissionEdit({ permission }: { permission: any }) {
    const { data, setData, put, processing, errors } = useForm({ name: permission.name ?? '', slug: permission.slug ?? '' });
        ...
        <AdminLayout title="Edit Permission">
            <div className="mx-auto w-full max-w-3xl space-y-4">
                ...
                <form onSubmit={onSubmit} className="space-y-5 rounded-xl border p-6 bg-white">
                    ...
                    <div>
                        <button type="submit" disabled={processing} className="rounded-md bg-black px-5 py-2.5 text-sm font-medium text-white disabled:opacity-50">Update</button>
                    </div>
                </form>
            </div>
        </AdminLayout>
