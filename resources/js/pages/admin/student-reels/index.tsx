...
export default function StudentReelsIndex({ reels, stats }: { reels: PaginatedData<StudentReel>; stats: Stats }) {
    ...
    const onDelete = async (id: number) => {
        ...
        router.delete(`/admin/student-reels/${id}`);
    };

    return (
        <AdminLayout title="ريلز الطلاب">
            <div className="space-y-6">
                <section className="rounded-[28px] border border-orange-100 bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_52%,#fff1e6_100%)] p-6 shadow-sm">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="text-right">
                            <div className="text-xs font-bold tracking-[0.3em] text-orange-500">قصص وتجارب</div>
                            <h1 className="mt-2 text-2xl font-black text-slate-900">إدارة ريلز الطلاب</h1>
                            <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-600">
                                اعرض فيديوهات قصيرة تعبّر عن تجربة الطلاب بصورة ممتعة وسهلة المشاهدة، مع تنظيم
                                الغلاف والاقتباس والترتيب.
                            </p>
                        </div>

                        <Link
                            href="/admin/student-reels/create"
                ...
                <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-6 py-4 text-right">
                        <h2 className="text-lg font-bold text-slate-900">قائمة ريلز الطلاب</h2>
                        <p className="mt-1 text-sm text-slate-500">
                            راجع بيانات الطالب، الغلاف، الفيديو، والاقتباس القصير لكل ريل.
                        </p>
                    </div>
                        ...
                        <table className="min-w-full text-sm">
                            <thead className="bg-slate-50 text-right text-slate-600">
                                <tr>
                                    ...
                                    <th className="px-4 py-3">الاقتباس</th>
                                ...
                                {reels.data.map((reel) => (
                                    <tr key={reel.id} className="border-t border-slate-100 align-top">
                                        ...
                                        <td className="px-4 py-4 text-right">
                                            <div className="font-semibold text-slate-900">{reel.student_name}</div>
                                            {reel.student_title && (
                                                <div className="mt-1 text-xs text-slate-500">{reel.student_title}</div>
                                            )}
                                            {reel.student_age && (
                                                <div className="mt-1 text-xs text-slate-500">{reel.student_age} سنة</div>
                                            )}
                                        </td>
                                        ...
                                        <td className="max-w-sm px-4 py-4 text-slate-600">
                                            <span className="line-clamp-3 leading-7">
                                                {reel.quote || 'لا يوجد اقتباس مضاف.'}
                                            </span>
                                        </td>
                                        <td className="px-4 py-4">
                                            <span
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center justify-start gap-3">
                                                <Link
                                                    href={`/admin/student-reels/${reel.id}/edit`}
                                ...
                                {reels.data.length === 0 && (
                                    <tr>
                                        <td colSpan={8} className="px-4 py-10 text-center text-slate-500">
                                            لا توجد ريلز طلاب مضافة حاليًا.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
