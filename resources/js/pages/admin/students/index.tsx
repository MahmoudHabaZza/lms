            ...
            <div className="space-y-6">
                <section className="rounded-[28px] border border-orange-100 bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_52%,#fff1e6_100%)] p-6 shadow-sm">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="text-right">
                            <div className="text-xs font-bold tracking-[0.3em] text-orange-500">إدارة الحسابات التعليمية</div>
                            <h1 className="mt-2 text-2xl font-black text-slate-900">قسم الطلاب</h1>
                            <p className="mt-2 text-sm leading-7 text-slate-600">
                                من هنا يمكنك إدارة حسابات الطلاب، تفعيلها، وإسناد الكورسات المناسبة لكل طالب.
                            </p>
                        </div>
                        <Link
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    <div className="rounded-3xl border border-orange-100 bg-white p-5 shadow-sm">
                        <div className="text-sm font-semibold text-slate-500">إجمالي الطلاب</div>
                        <div className="mt-3 text-3xl font-black text-slate-900">{stats.total}</div>
                    </div>
                    <div className="rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm">
                        <div className="text-sm font-semibold text-slate-500">الحسابات المفعلة</div>
                        <div className="mt-3 text-3xl font-black text-emerald-700">{stats.active}</div>
                    </div>
                    <div className="rounded-3xl border border-rose-100 bg-white p-5 shadow-sm">
                        <div className="text-sm font-semibold text-slate-500">الحسابات غير المفعلة</div>
                        <div className="mt-3 text-3xl font-black text-rose-700">{stats.inactive}</div>
                    </div>
                </div>
                ...
                <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="grid gap-4 lg:grid-cols-[1fr_220px_auto]">
                        ...
                        <select
                            ...
                            className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-right text-sm outline-none transition focus:border-orange-400 focus:bg-white focus:ring-4 focus:ring-orange-100"
                        >
                ...
                <div className="space-y-4">
                    {students.data.map((student) => (
                        <article key={student.id} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                <div className="text-right">
                                    <div className="flex flex-wrap items-center justify-end gap-2">
                                        ...
                                        >
                                            {student.is_active ? 'مفعل' : 'غير مفعل'}
                                        </span>
                                    </div>
                                    ...
                                    <div className="mt-1 text-sm text-slate-500">
                                        {student.phone_number || 'لا يوجد رقم هاتف مسجل'} • تاريخ الإضافة: {student.created_at || '-'}
                                    </div>
                                ...
                                <div className="flex items-center justify-end gap-3">
                                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-center">
                                        <div className="text-xs font-semibold text-slate-500">الكورسات</div>
                                        <div className="mt-1 text-lg font-black text-slate-900">{student.assigned_courses_count}</div>
                                    </div>
                                    <Link
                                        href={`/admin/students/${student.id}/edit`}
                                        className="inline-flex items-center gap-2 rounded-2xl border border-orange-200 bg-orange-50 px-4 py-3 text-sm font-semibold text-orange-700 transition hover:border-orange-300 hover:bg-orange-100"
                                    >
