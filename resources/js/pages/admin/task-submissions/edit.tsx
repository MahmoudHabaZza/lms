...
export default function TaskSubmissionEdit({
    submission,
    students,
    tasks,
}: {
    submission: { id: number; task_id: number | null; student_id: number | null; feedback: string | null; score: number | null; status: string; submission_file_url: string | null };
    students: { id: number; name: string }[];
    tasks: { id: number; title: string }[];
}) {
    const { data, setData, post, processing, errors } = useForm({
        task_id: submission.task_id ? String(submission.task_id) : '',
    ...
    const onSubmit = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        post(`/admin/task-submissions/${submission.id}`, { forceFormData: true });
    };

    return (
        <AdminLayout title="تعديل تسليم مهمة">
            <div className="mx-auto w-full max-w-5xl space-y-5">
                <div className="flex items-center justify-between">
                    <div className="text-right"><h1 className="text-2xl font-black text-slate-900">تعديل تسليم المهمة</h1></div>
                ...
                <form onSubmit={onSubmit} className="grid gap-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div>
                        ...
                        {submission.submission_file_url && <a href={submission.submission_file_url} target="_blank" rel="noreferrer" className="mt-2 inline-flex text-sm font-semibold text-blue-600 hover:underline">فتح الملف الحالي</a>}
